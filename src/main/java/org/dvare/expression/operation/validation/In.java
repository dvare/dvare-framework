...
@org.dvare.annotations.Operation(type = OperationType.VALIDATION, symbols = {"IN", "In", "in"})
    ...
    private void testInOperation(String[] tokens, int pos) throws ExpressionParseException {
        ...
        if (right instanceof VariableExpression) {
            ...
            if (!variableExpression.isList()) {
                message = String.format("List ValidationOperation %s not possible on type %s near %s", this.getClass().getSimpleName(), variableExpression.getType().getDataType(), ExpressionTokenizer.toString(tokens, pos + 2));
